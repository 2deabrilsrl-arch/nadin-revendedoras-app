...
function generarHTMLEmail(data: any, productosAgrupados: ProductoAgrupado[]) {
  const { revendedora, pedidos, totales, formaPago, tipoEnvio, transporteNombre, linkMagico } = data;
...
<body>
  ...
  <!-- ✅ SECCIÓN: BOTÓN DE ARMADO CONSOLIDADO -->
  ${linkMagico ? `
  <div class="link-section">
    <h2 style="margin-top: 0; color: #856404;">📦 ¡Armá la consolidación en la app!</h2>
    <p style="font-size: 16px;">
      Hacé click en el botón para armar <strong>todos los pedidos</strong> en una sola pantalla.<br>
      <strong>No necesitás login</strong> - el link es único y seguro.
    </p>
    
    <a href="${linkMagico}" class="cta-button">
      📦 ARMAR CONSOLIDACIÓN (${pedidos.length} pedidos)
    </a>
    
    <p style="font-size: 14px; margin-top: 20px;">
      💬 Podés <strong>chatear con ${revendedora.name}</strong> mientras armás<br>
      ✅ El sistema actualiza todo automáticamente cuando finalizás<br>
      📋 Verás todos los productos con los datos de cada clienta
    </p>
  </div>
  ` : ''}
  ...
  <div class="footer">
    ...
    ${linkMagico ? `
    <p style="font-size: 12px; margin-top: 15px; background: #e7f3ff; padding: 10px; border-radius: 4px;">
      💡 <strong>Tip:</strong> Usá el botón de arriba para armar toda la consolidación en la app.<br>
      Podés chatear con la revendedora y marcar productos en tiempo real.
    </p>
    ` : ''}
...
export async function sendConsolidacionEmailMejorado(data: any) {
  ...
  if (linkMagico) {
    console.log(`🔗 Link mágico incluido: ${linkMagico}`);
    console.log(`📦 Consolidación de ${pedidos.length} pedidos`);
  }
}
...
/**
 * NUEVO: Enviar notificación de mensaje por email
 */
export async function sendMensajeNotificacion(data: {
  destinatarioEmail: string;
  destinatarioNombre: string;
  remitenteNombre: string;
  remitenteTipo: 'revendedora' | 'vendedora';
  mensaje: string;
  consolidacionId: string;
  linkChat?: string;
}) {
  const { destinatarioEmail, destinatarioNombre, remitenteNombre, remitenteTipo, mensaje, linkChat } = data;

  const esParaVendedora = remitenteTipo === 'revendedora';
  const asunto = esParaVendedora 
    ? `💬 Mensaje de ${remitenteNombre} sobre su pedido`
    : `💬 Respuesta de Nadin sobre tu pedido`;
...
<body>
  <div class="header">
    <h1>💬 Nuevo Mensaje</h1>
    <p style="margin: 0; opacity: 0.9;">Nadin Revendedoras</p>
  </div>
  ...
  <p>
    ${esParaVendedora 
      ? `<strong>${remitenteNombre}</strong> te envió un mensaje sobre su pedido consolidado:`
      : `<strong>Nadin</strong> respondió sobre tu pedido consolidado:`
    }
  </p>
  ...
  <div style="text-align: center;">
    ${linkChat ? `
    <a href="${linkChat}" class="cta-button">
      💬 Ver Conversación en la App
    </a>
    ` : `
    <p style="color: #6c757d; font-size: 14px;">
      Ingresá a la app para responder este mensaje
    </p>
    `}
  </div>
  ...
  <p style="font-size: 14px; color: #6c757d; margin-top: 30px;">
    ${esParaVendedora 
      ? 'Podés responder desde la app de armado de consolidación.'
      : 'Podés ver la conversación completa en tu app de revendedoras.'
    }
  </p>
  ...
  try {
    ...
    console.log(`✅ Email de notificación enviado a ${destinatarioEmail}`);
  } catch (error) {
